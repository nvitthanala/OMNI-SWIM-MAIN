...
2) Scoring Engine (`backend/point_calculator.py`)
...
- Settings file: the server writes the active workspace scoring to `data/scoring_settings.json` before invoking the calculator. The Python process also respects `OMNI_DATA_DIR` when set.
...
- `src/components/ScoringSettingsPanel.tsx` and `ScoringSettingsModal.tsx` allow editing scoring settings. Saving writes to the workspace object; the server writes workspace settings to `data/scoring_settings.json` before invoking the Python calculator.
...
- Endpoint `/api/parse-pdf` accepts a base64 PDF, runs `backend/pdf_parser.py`, writes `data/scoring_settings.json`, then runs `backend/point_calculator.py` and returns normalized results.
- Workspace persistence lives in `data/meets.json` (legacy `meets.json` at repo root is migrated on startup when present).

5) Utilities & Helpers

- `utils/compare_all_teams.py` — helper script to extract official team totals from last pages and compare against our computed totals. Now does fuzzy alignment of names.

6) Testing & Checkpoints

- Checkpoints saved under `archive/checkpoints/` with snapshots of edited files.
...
- Open the Scoring Settings modal in the app and edit the `Scoring Points`, `Relay Multiplier`, and caps. Saving updates workspace settings and will be used by the next parse operation (server writes settings to `data/scoring_settings.json`).
